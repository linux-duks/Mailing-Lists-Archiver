...
LISTS_OF_INTEREST = ["org.freedesktop.lists.amd-gfx", "org.freedesktop.lists.intel-gfx", "org.kernel.vger.linux-iio", "org.kernel.vger.rust-for-linux"]
...
for i in range(1, len(LISTS_OF_INTEREST)):
    new_list_df = pl.read_parquet("/input/list=" + LISTS_OF_INTEREST[i] + "/list_data.parquet")
...
def retrieve_reviewers_and_testers(sorted_df):
    ...
    while thisDate < LASTCOMMITDATE :
        ...
        while window_end < len(sorted_df) - 1 and maxDate >= sorted_df[window_end+1]["date"][0]:
            window_end+=1
        ...
        while window_begin < len(sorted_df) -1 and minDate > sorted_df[window_begin]["date"][0]: 
            window_begin += 1
        ...
        for mailing_list in results:
            results[mailing_list]["reviewed_points"].append(results[mailing_list]["running_reviewed"])
            results[mailing_list]["tested_points"].append(results[mailing_list]["running_tested"])
            results[mailing_list]["any_points"].append(results[mailing_list]["running_reviewed"] + results[mailing_list]["running_tested"])
